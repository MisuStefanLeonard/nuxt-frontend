...
async function getCurrentTipLinie() {
    ...
    if (response === null) {
        ...
        formData.value.nume_ro = response.numeTipLinieJsonDto.nume_ro,
        formData.value.nume_en = response.numeTipLinieJsonDto.nume_en,
        ...
        originalTipLinie.value = JSON.parse(JSON.stringify(tipLinieObj))
    }
}
...
async function saveTipLinieModification(){
    ...
    if(isValidForm.valid){
        ...
        if(JSON.stringify(modifiedTipLinie) === JSON.stringify(originalTipLinie.value)){
